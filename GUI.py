...
class Gui:
    def __init__(self):
        ...
        layout = [  
                [sg.Text('\t      PenTasker', justification='center', size=(100, 1), font='Helvetica 14'), sg.Text("",visible=False, key="-PKTS-"), sg.Button('Import\nProject', button_color=('white', '#3971c6'), size=(10,2), key='-BROWSE-')], # ,file_types=(("PTSK Files", "*.ptsk"),), 
                [sg.Frame('',[
                    [sg.Button('Select all'), sg.Button('Deselect all')],
                    [sg.Text('')],
                    [sg.Text(' '*25), sg.Text('URL / IP: '), sg.Input('', size=(40,1), justification='center', key='-URL-'),sg.Button('', image_data=help_b64,button_color=(sg.theme_background_color(),sg.theme_background_color()),border_width=0, key='?'),sg.Text(' '*25, key='-ERROR-')],
